...
create_intercept_env <- function(parent, intercepts) {
  ...
  if (length(intercepts)) {
    env_bind(e, `::` = double_colon_intercepter)
    fn_env(e$`::`) <- e

    env_bind(e, `:::` = triple_colon_intercepter)
    fn_env(e$`:::`) <- e
  }
...
generate_mask <- function(envir, hidden) {
  ...
  f <- function(expr, .binding_env = caller_env(), .expr_text = deparse(substitute(expr))) {
    on.exit(copy_bindings(masked_env, binding_env))
    mask_message(.expr_text)
    out <- eval(call2(withVisible, substitute(expr)), masked_env)
...
print.mask <- function(x) {
  cat(paste("Mask",
            env_name(attr(m, "masking_environment")),
            env_name(attr(m, "enclosing_environment")),
            sep = "\n"))
}
